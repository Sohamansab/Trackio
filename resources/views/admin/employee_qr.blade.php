...
<div class="container-fluid">
    ...
    @if($employee->qr_code)
        ...
        ">
            ...
            <div style="text-align:center; margin-bottom:15px;">
                <div style="font-size:24px; font-weight:bold;">{{ config('app.name', 'Trackio') }}</div>
            ...
            <div style="font-size:10px; line-height:1.6; margin-bottom:15px;">
                <div><strong>Department:</strong> {{ $employee->department->dept_name ?? 'N/A' }}</div>
                <div><strong>Designation:</strong> {{ $employee->designation->designation_name ?? 'N/A' }}</div>
                <div><strong>Joining:</strong> {{ $employee->joining_date ? date('d M Y', strtotime($employee->joining_date)) : 'N/A' }}</div>
            </div>

            <!-- QR Code Section -->
            <div style="text-align:center; margin-bottom:10px;">
                <img id="card-qr" src="{{ asset($employee->qr_code_path) }}" alt="QR Code" style="width:80px; height:80px;">
            </div>
    ...
    @else
        <div class="alert alert-warning">
            No QR code generated yet. <a href="{{ route('admin.employee.qr.generate', $employee->emp_id) }}">Generate QR Code</a>
        </div>
    @endif
</div>
